...
# Returns a sequence of simulation from the state entered as a parameter to a terminal state
# Used for MC learning to learn episodes are return them
...
# Simulation function for TD-learning
# Based on the state entered, return the next state, along with reward
# Also return if the state is a terminal state
def simulate_one_step(state, action, userCard, dealCard, ccards, stand):
    ...
    while True:
        ...
        # calculate reward
        if not (gameover or stand):
            # Returning non-terminal state
            # Avoiding returning same state if not terminal
            ...
            if userSum == dealSum:
                ...
                reward = -1
            # Returning the terminal state with 'True' and reward
...
# Reward to go function for MC learning
# Gets the index of where the state is located in the episode
# And calculates reward from that state to the terminal state
def reward_to_go(index, gamma, episode):
    # Placeholder, need to implement the right calculation
    reward = 0
    power = 0
    for i in range(index, len(episode)):
        reward += ((gamma ** power) * episode[i][1])
        power += 1
    return reward
...
def MC_Policy_Evaluation(policy, states, gamma, MCvalues, G):
    ...
    for simulation in range(50):
        ...
        index = 0
        # For each episode pass in the index of the state inside the episode
        # In order to calculate reward of that state easily
        for e in episode:
            G[e[0]].append(reward_to_go(index, gamma, episode))
...
def TD_Policy_Evaluation(policy, states, gamma, TDvalues, NTD):
    for simulation in range(50):
        ...
        state = make_state(userSum, userA, dealFirst, dealAFirst)
        s = (state, 0, False)
        # Pass in a tuple of state, reward, if terminal state
        while True:
            next_s = simulate_one_step(s[0], None, userCard, dealCard, ccards, None)
            if next_s[0] == s[0]:
                # In case the initial state has already gameover, the same state is returned
                # Implementation detail based on how my simulation function is implemented
                NTD[s[0]] += 1
                TDvalues[s[0]] = TDvalues[s[0]] + ((10/(9+NTD[s[0]])) * (next_s[1] - TDvalues[s[0]]))
                break
            elif next_s[2] == True:
                # If the next is terminal, we will have to set current and next together
                NTD[s[0]] += 1
                TDvalues[s[0]] = TDvalues[s[0]] + ((10/(9+NTD[s[0]]))*(s[1]+(gamma*TDvalues[next_s[0]]) - TDvalues[s[0]]))
                NTD[next_s[0]] += 1
                TDvalues[next_s[0]] = TDvalues[next_s[0]] + ((10/(9+NTD[next_s[0]])) * (next_s[1] - TDvalues[next_s[0]]))
                break
            else:
                # Setting the next to current state case
                NTD[s[0]] += 1
                TDvalues[s[0]] = TDvalues[s[0]] + ((10 / (9 + NTD[s[0]])) * (s[1] + (gamma * TDvalues[next_s[0]]) - TDvalues[s[0]]))
                s = next_s

# Pick action function based on the slides
...
# Simualation function for Q-Learning
# Gets a parameter saying hit or stand
# After user stands, if usersum <= 21, dealer hits till one player loses or players draw
def simulate_one_step_action(state, action, userCard, dealCard, ccards):
    userSum = state[0]
    userA = state[1]
    dealSum = state[2]
    dealA = state[3]
    ...
    if not (gameover or stand):
        ...
        if userSum == dealSum:
            ...
            reward = -1
        # Returning terminal states
        return state, reward, True

def Q_Learning(states, gamma, Qvalues, NQ):
    for simulation in range(50):
        # generate random game or arbitrary state
        userCard = []
        dealCard = []
        ccards = copy.copy(cards)
        userSum, userA, dealSum, dealA, dealFirst, dealAFirst = initGame(ccards, userCard, dealCard)
        ...
        eps = 0.25
        while True:
            a = pick_action(s[0], eps, Qvalues)
            next_s = simulate_one_step_action(s[0], a, userCard, dealCard, ccards)
            if next_s[2] == True:
                # Case where next state is terminal, we need to break the iteration
                NQ[s[0]] += 1
                Qvalues[s[0]][a] = Qvalues[s[0]][a] + ((10 / (9 + NQ[s[0]])) * (
                            next_s[1] - Qvalues[s[0]][a]))
                break
            else:
                # Next is non-terminal, and we set current to next for the next iteration
                NQ[s[0]] += 1
                Qvalues[s[0]][a] = Qvalues[s[0]][a] + ((10 / (9 + NQ[s[0]])) * (
                            s[1] + (gamma * max(Qvalues[next_s[0]][0], Qvalues[next_s[0]][1])) - Qvalues[s[0]][a]))
                s = next_s
